...
class UpgradeNotesTest extends TestCase
{
    ...
    public function testPrintUpgradeLink($direction, $from, $to)
    {
        ...
        if ($direction === 'up') {
            ...
            $this->assertContains("https://github.com/zivvio/zivvio/blob/$to/framework/UPGRADE.md", implode("\n", $io->messages));
        } else {
            if ($from === 'dev-main') {
                $from = 'main';
            }
            $this->assertContains("https://github.com/zivvio/zivvio/blob/$from/framework/UPGRADE.md", implode("\n", $io->messages));
        }
    }
    ...
    {
        $notes = $this->invokeMethod($this->getPlugin(), 'findUpgradeNotes', ['zivvio/zivvio', '2.0.0-alpha']);
...
STRING
    ...
    {
        $notes = $this->invokeMethod($this->getPlugin(), 'findUpgradeNotes', ['zivvio/zivvio', '2.0.10']);
        $this->assertEquals(<<<STRING
Upgrade from Yii 2.0.14
-----------------------
...
STRING
    ...
    {
        $notes = $this->invokeMethod($this->getPlugin(), 'findUpgradeNotes', ['zivvio/zivvio', '2.0.13.1']);
        $this->assertEquals(<<<STRING
Upgrade from Yii 2.0.14
-----------------------
...
STRING
    ...
    {
        $notes = $this->invokeMethod($this->getPlugin(), 'findUpgradeNotes', ['zivvio/zivvio', '2.0.12.1']);
        $this->assertEquals(<<<STRING
Upgrade from Yii 2.0.14
-----------------------
